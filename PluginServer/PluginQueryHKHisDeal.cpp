...
//tomodify 2
#define PROTO_ID_QUOTE			PROTO_ID_TDHK_QUERY_HIS_DEAL
typedef CProtoQueryHKHisDeal	CProtoQuote;

//////////////////////////////////////////////////////////////////////////
CPluginQueryHKHisDeal::CPluginQueryHKHisDeal()
{	
	m_pTradeOp = NULL;
	m_pTradeServer = NULL;
...
void CPluginQueryHKHisDeal::Init(CPluginHKTradeServer* pTradeServer, ITrade_HK*  pTradeOp)
{
...
void CPluginQueryHKHisDeal::Uninit()
{
	if ( m_pTradeServer != NULL )
	{
		m_pTradeServer = NULL;
...
void CPluginQueryHKHisDeal::SetTradeReqData(int nCmdID, const Json::Value &jsnVal, SOCKET sock)
{
	...
	//tomodify 3
	QueryHKHisDealReqBody &body = req.body;	
	...
	int nReqResult = 0;
	bool bRet = m_pTradeOp->QueryHisDealList((Trade_Env)body.nEnvType, (UINT32*)&pReq->dwLocalCookie, strStartDate.c_str(), strEndDate.c_str(), &nReqResult);
...
bool CPluginQueryHKHisDeal::DoDeleteReqData(StockDataReq* pReq)
{
...
void CPluginQueryHKHisDeal::NotifyOnQueryHKHisDeal(Trade_Env enEnv, UINT32 nCookie, INT32 nCount, const Trade_DealItem* pArrDeal)
{
	...
	if ( nCount > 0 && pArrDeal )
	{
		...
		{			
			...
			item.nContraBrokerID = deal.nContraBrokerID;
			item.strContraBrokerName = deal.szContraBrokerName;

			if (IsFitFilter(item, setCode))
			{
				ack.body.vtHisDeal.push_back(item);
			}
		}
	}
...
void CPluginQueryHKHisDeal::NotifySocketClosed(SOCKET sock)
{
	DoClearReqInfo(sock);
}

void CPluginQueryHKHisDeal::OnTimeEvent(UINT nEventID)
{
...
void CPluginQueryHKHisDeal::OnMsgEvent(int nEvent,WPARAM wParam,LPARAM lParam)
{
...
void CPluginQueryHKHisDeal::HandleTimeoutReq()
{
...
void CPluginQueryHKHisDeal::HandleTradeAck(TradeAckType *pAck, SOCKET sock)
{
...
void CPluginQueryHKHisDeal::SetTimerHandleTimeout(bool bStartOrStop)
{
...
void CPluginQueryHKHisDeal::ClearAllReqAckData()
{
	VT_REQ_TRADE_DATA::iterator it_req = m_vtReqData.begin();
	for ( ; it_req != m_vtReqData.end(); )
	{
...
void CPluginQueryHKHisDeal::DoClearReqInfo(SOCKET socket)
{
...
void CPluginQueryHKHisDeal::DoGetFilterCode(const std::string& strFilter, std::set<std::wstring>& setCode)
{
	setCode.clear();
	if (strFilter.empty())
	{
...
bool CPluginQueryHKHisDeal::IsFitFilter(const TradeAckItemType& AckItem, const std::set<std::wstring>& vtCode)
{
