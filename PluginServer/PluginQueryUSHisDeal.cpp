...
//tomodify 2
#define PROTO_ID_QUOTE			PROTO_ID_TDUS_QUERY_HIS_DEAL
typedef CProtoQueryUSHisDeal	CProtoQuote;

//////////////////////////////////////////////////////////////////////////
CPluginQueryUSHisDeal::CPluginQueryUSHisDeal()
{	
	m_pTradeOp = NULL;
	m_pTradeServer = NULL;
...
void CPluginQueryUSHisDeal::Init(CPluginUSTradeServer* pTradeServer, ITrade_US*  pTradeOp)
{
...
void CPluginQueryUSHisDeal::Uninit()
{
	if ( m_pTradeServer != NULL )
	{
		m_pTradeServer = NULL;
...
void CPluginQueryUSHisDeal::SetTradeReqData(int nCmdID, const Json::Value &jsnVal, SOCKET sock)
{
	...
	//tomodify 3
	QueryUSHisDealReqBody &body = req.body;	
	...
	int nReqResult = 0;
	bool bRet = m_pTradeOp->QueryHisDealList((UINT32*)&pReq->dwLocalCookie, strStartDate.c_str(), strEndDate.c_str(), &nReqResult);
...
bool CPluginQueryUSHisDeal::DoDeleteReqData(StockDataReq* pReq)
{
...
void CPluginQueryUSHisDeal::NotifyOnQueryUSHisDeal(Trade_Env enEnv, UINT32 nCookie, INT32 nCount, const Trade_DealItem* pArrDeal)
{
...
void CPluginQueryUSHisDeal::NotifySocketClosed(SOCKET sock)
{
	DoClearReqInfo(sock);
}

void CPluginQueryUSHisDeal::OnTimeEvent(UINT nEventID)
{
...
void CPluginQueryUSHisDeal::OnMsgEvent(int nEvent,WPARAM wParam,LPARAM lParam)
{
...
void CPluginQueryUSHisDeal::HandleTimeoutReq()
{
...
void CPluginQueryUSHisDeal::HandleTradeAck(TradeAckType *pAck, SOCKET sock)
{
...
void CPluginQueryUSHisDeal::SetTimerHandleTimeout(bool bStartOrStop)
{
...
void CPluginQueryUSHisDeal::ClearAllReqAckData()
{
	VT_REQ_TRADE_DATA::iterator it_req = m_vtReqData.begin();
	for ( ; it_req != m_vtReqData.end(); )
	{
...
void CPluginQueryUSHisDeal::DoClearReqInfo(SOCKET socket)
{
...
void CPluginQueryUSHisDeal::DoGetFilterCode(const std::string& strFilter, std::set<std::wstring>& setCode)
{
	setCode.clear();
	if (strFilter.empty())
	{
...
bool CPluginQueryUSHisDeal::IsFitFilter(const TradeAckItemType& AckItem, const std::set<std::wstring>& vtCode)
{
