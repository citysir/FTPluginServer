...
//tomodify 2
#define PROTO_ID_QUOTE			PROTO_ID_TDUS_SUB_ORDER_DEAL
typedef CProtoSubUSOrderDeal	CProtoQuote;

//////////////////////////////////////////////////////////////////////////

CPluginSubUSOrderDeal::CPluginSubUSOrderDeal()
{	
	m_pTradeOp = NULL;
	m_pTradeServer = NULL;
	m_pOrderPusher = NULL;
...
void CPluginSubUSOrderDeal::Init(CPluginUSTradeServer* pTradeServer, ITrade_US* pTradeOp, CPluginPushUSOrder* pOrderPusher, CPluginPushUSDeal* pDealPusher)
{
...
void CPluginSubUSOrderDeal::Uninit()
{
	if ( m_pTradeServer != NULL )
	{
		m_pTradeServer = NULL;
...
void CPluginSubUSOrderDeal::SetTradeReqData(int nCmdID, const Json::Value &jsnVal, SOCKET sock)
{
...
void CPluginSubUSOrderDeal::SetTradeReqData(const TradeReqType &req, SOCKET sock)
{
	...
	if (body.strOrderID.empty())//全部订阅
	{
		...
		int nOrderIDCount = 0;
		if (bFirstPush && m_pTradeOp->GetOrderIDList(NULL, nOrderIDCount))
		{
			UINT64* pOrderIDList = new UINT64[nOrderIDCount];
			if (m_pTradeOp->GetOrderIDList(pOrderIDList, nOrderIDCount))
			{
		...
		NotifyOnSubUSOrder(eEnv, body.nCookie, body.strOrderID, strSubOrderSuc, strSubDealSuc);
	}
	else
	{
...
bool CPluginSubUSOrderDeal::DoDeleteReqData(StockDataReq* pReq)
{
...
void CPluginSubUSOrderDeal::NotifyOnSubUSOrder(Trade_Env enEnv, UINT32 nCookie, std::string strOrderID,
...
void CPluginSubUSOrderDeal::NotifySocketClosed(SOCKET sock)
{
	DoClearStocketInfo(sock);
}

void CPluginSubUSOrderDeal::ClearSubOrderInfo(UINT64 nOrderID)
{
...
void CPluginSubUSOrderDeal::ClearSubDealInfo(UINT64 nOrderID)
{
...
void CPluginSubUSOrderDeal::GetSubOrderSocket(Trade_Env enEnv, UINT64 nOrderID, std::vector<SOCKET> &vtSock)
{
	vtSock.clear();
...
void CPluginSubUSOrderDeal::GetSubDealSocket(Trade_Env enEnv, UINT64 nOrderID, std::vector<SOCKET> &vtSock)
{
	vtSock.clear();
...
void CPluginSubUSOrderDeal::NotifyUnLockTrade(SOCKET sock, Trade_SvrResult enSvrRet)
{
...
void CPluginSubUSOrderDeal::OnTimeEvent(UINT nEventID)
{
...
void CPluginSubUSOrderDeal::OnMsgEvent(int nEvent,WPARAM wParam,LPARAM lParam)
{

}

void CPluginSubUSOrderDeal::HandleTimerClearSubInfo()
{
	...
	for (; iter != m_mapEnvType.end(); ++iter)
	{
		...
		bool bRet = m_pTradeOp->GetOrderItem(iter->first, orderItem);
...
void CPluginSubUSOrderDeal::HandleTradeAck(TradeAckType *pAck, SOCKET sock)
{
...
void CPluginSubUSOrderDeal::SetTimerClearSubInfo(bool bStartOrStop)
{
...
void CPluginSubUSOrderDeal::HandleReqUnlockTimeout()
{
	CHECK_RET(m_pTradeOp, NORET);
...
void CPluginSubUSOrderDeal::SetTimerReqUnlockTimeout(bool bStartOrStop)
{
...
void CPluginSubUSOrderDeal::ClearAllReqAckData()
{
...
void CPluginSubUSOrderDeal::DoClearStocketInfo(SOCKET socket)
{
...
bool CPluginSubUSOrderDeal::IsSubOrder(UINT64 nOrderID)
{
	SET_ORDER_SOCKET& setOrder = m_mapSubOrder;
	auto itOrder = setOrder.begin();
	while (itOrder != setOrder.end())
...
bool CPluginSubUSOrderDeal::IsSubDeal(UINT64 nOrderID)
{
	SET_DEAL_SOCKET& setDeal = m_mapSubDeal;
	auto itDeal = setDeal.begin();
	while (itDeal != setDeal.end())
...
void CPluginSubUSOrderDeal::ClearOrderEnvInfo(UINT64 nOrderID)
{
...
void CPluginSubUSOrderDeal::ParseOrderIDStr(std::string strOrderID, std::vector<UINT64> &vOrderID)
{
	vOrderID.clear();
	CString strDiv = _T(",");
...
bool CPluginSubUSOrderDeal::SubOrderDeal(SOCKET sock, Trade_Env enEnv, UINT64 nOrderID, bool bSubOrder, bool bSubDeal, bool bFirstPush)
{	
	Trade_OrderStatus eOrderStatus = Trade_OrderStatus(0);
	bool bRet = m_pTradeOp->GetOrderStatus(nOrderID, eOrderStatus);
...
void CPluginSubUSOrderDeal::PushOrder(SOCKET sock, Trade_Env enEnv, UINT64 nOrderID)
{
	CHECK_RET(m_pTradeOp && m_pOrderPusher, NORET);

	//订阅首推订单状态
	Trade_OrderItem orderItem = {};
	bool bGetOrderItm = m_pTradeOp->GetOrderItem(nOrderID, orderItem);
...
void CPluginSubUSOrderDeal::PushDeal(SOCKET sock, Trade_Env enEnv, UINT64 nOrderID)
{
	CHECK_RET(m_pTradeOp && m_pOrderPusher, NORET);

	//订阅首推该订单之前成交记录
	int nCount = 0;
	m_pTradeOp->GetDealItemsByOrderID(nOrderID, NULL, nCount);
	if (nCount > 0)
	{
		Trade_DealItem* pDealItems = new Trade_DealItem[nCount];
		CHECK_RET(pDealItems, NORET);
		if (m_pTradeOp->GetDealItemsByOrderID(nOrderID, &pDealItems, nCount))
		{
