...
class CPluginPushHKDeal;

class CPluginSubHKOrderDeal : public CTimerWndInterface, public CMsgHandlerEventInterface
{
	typedef SubHKOrderDeal_Req	TradeReqType;
	typedef SubHKOrderDeal_Ack	TradeAckType;

public:
	CPluginSubHKOrderDeal();
	virtual ~CPluginSubHKOrderDeal();
	
	void Init(CPluginHKTradeServer* pTradeServer, ITrade_HK* pTradeOp, 
		CPluginPushHKOrder* pOrderPusher, CPluginPushHKDeal* pDealPusher);
	void Uninit();	
	...
	void NotifyOnSubHKOrder(Trade_Env enEnv, UINT32 nCookie, std::string strOrderID,
...
protected:
	...
	CPluginPushHKOrder		*m_pOrderPusher;
	CPluginPushHKDeal		*m_pDealPusher;
