...
//tomodify 2
#define PROTO_ID_QUOTE			PROTO_ID_TDHK_QUERY_HIS_ORDER
typedef CProtoQueryHKHisOrder	CProtoQuote;

//////////////////////////////////////////////////////////////////////////
CPluginQueryHKHisOrder::CPluginQueryHKHisOrder()
{	
	m_pTradeOp = NULL;
	m_pTradeServer = NULL;
...
void CPluginQueryHKHisOrder::Init(CPluginHKTradeServer* pTradeServer, ITrade_HK*  pTradeOp)
{
...
void CPluginQueryHKHisOrder::Uninit()
{
	if ( m_pTradeServer != NULL )
	{
		m_pTradeServer = NULL;
...
void CPluginQueryHKHisOrder::SetTradeReqData(int nCmdID, const Json::Value &jsnVal, SOCKET sock)
{
	...
	//tomodify 3
	QueryHKHisOrderReqBody &body = req.body;	
	...
	int nReqResult = 0;
	bool bRet = m_pTradeOp->QueryHisOrderList((Trade_Env)body.nEnvType, (UINT32*)&pReq->dwLocalCookie, strStartDate.c_str(), strEndDate.c_str(), &nReqResult);
...
bool CPluginQueryHKHisOrder::DoDeleteReqData(StockDataReq* pReq)
{
...
void CPluginQueryHKHisOrder::NotifyOnQueryHKHisOrder(Trade_Env enEnv, UINT32 nCookie, INT32 nCount, const Trade_OrderItem* pArrOrder)
{
...
void CPluginQueryHKHisOrder::NotifySocketClosed(SOCKET sock)
{
	DoClearReqInfo(sock);
}

void CPluginQueryHKHisOrder::OnTimeEvent(UINT nEventID)
{
...
void CPluginQueryHKHisOrder::OnMsgEvent(int nEvent,WPARAM wParam,LPARAM lParam)
{
...
void CPluginQueryHKHisOrder::HandleTimeoutReq()
{
...
void CPluginQueryHKHisOrder::HandleTradeAck(TradeAckType *pAck, SOCKET sock)
{
...
void CPluginQueryHKHisOrder::SetTimerHandleTimeout(bool bStartOrStop)
{
...
void CPluginQueryHKHisOrder::ClearAllReqAckData()
{
	VT_REQ_TRADE_DATA::iterator it_req = m_vtReqData.begin();
	for ( ; it_req != m_vtReqData.end(); )
	{
...
void CPluginQueryHKHisOrder::DoClearReqInfo(SOCKET socket)
{
...
void CPluginQueryHKHisOrder::DoGetFilterStatus(const std::string& strFilter, std::set<int>& setStatus)
{
	setStatus.clear();
	CString strDiv = _T(",");
...
void CPluginQueryHKHisOrder::DoGetFilterCode(const std::string& strFilter, std::set<std::wstring>& setCode)
{
	setCode.clear();
	if (strFilter.empty())
	{
...
bool CPluginQueryHKHisOrder::IsFitFilter(const TradeAckItemType& AckItem,
